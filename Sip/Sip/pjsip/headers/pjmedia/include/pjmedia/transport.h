...
struct pjmedia_transport
{
	...
	/* llz add pjmedia_encrypt_t */
	pjmedia_encrypt_t* encrypt_entity;
};
...
PJ_INLINE(pj_status_t) pjmedia_transport_send_rtp(pjmedia_transport *tp,
						  ...
						  pj_size_t size)
{
	//llz add encrypt_entity to encode data
	char* thisFile = "transport.h";
	PJ_LOG(4, (thisFile, "tp--->%p,", tp));
	if (tp->encrypt_entity 
		&& tp->encrypt_entity->session_key_len > 0
		&& tp->encrypt_entity->encode_sip_stream)
	{
		PJ_LOG(4, (thisFile, "Begin to encode rtp data. session key:%s,len:%d", tp->encrypt_entity->session_key, tp->encrypt_entity->session_key_len));
		PJ_LOG(4, (thisFile, "tp--->%p,tp->encrypt_entity--->%p,"
			"tp->encrypt_entity->encode_sip_stream--->%p,"
			"tp->encrypt_entity->session_key_len=%d",
			tp, tp->encrypt_entity, tp->encrypt_entity->encode_sip_stream, tp->encrypt_entity->session_key_len));
		unsigned char new_pkt[PJMEDIA_PKG_LEN];
		pj_bzero(new_pkt, PJMEDIA_PKG_LEN);
		pj_size_t new_size = PJMEDIA_PKG_LEN;
		unsigned char* key = (unsigned char*)tp->encrypt_entity->session_key;
		int key_len = tp->encrypt_entity->session_key_len;
		int res = tp->encrypt_entity->encode_sip_stream((const unsigned char*)pkt, (int)size, new_pkt, (int*)&new_size, key, key_len);
		if (res)
		{
			//如果加密失败，则不会加密， 直接发送源数据出去
			PJ_LOG(4, (thisFile, "Warring:Failed Encode sip rtp package."));
			return (*tp->op->send_rtp)(tp, pkt, size);
		}
		return (*tp->op->send_rtp)(tp, (void*)new_pkt, new_size);
	}else {

		if (tp) {
			PJ_LOG(4, (thisFile, "tp is null"));
		}
		else if (tp->encrypt_entity) {
			PJ_LOG(4, (thisFile, "tp->encrypt_entity is null"));
		}else if(tp->encrypt_entity->session_key_len) {
			PJ_LOG(4, (thisFile, "tp->encrypt_entity->session_key_len is null"));
		}
		return (*tp->op->send_rtp)(tp, pkt, size);
	}
	//llz add encrypt_entity to encode data
